...
- If no product URL, use a single-link structure with just the report link
...
- If no product URL, single-link structure with just the report link
...
Finally, print the jpub publish command using the **absolute path** to the date-prefix
(resolve via `pwd` or equivalent — the path must work from any terminal tab):
...
For example: `jpub /Users/jamiewatters/DevProjects/BOS-AI/progress/2026-04-11 --all --dry-run`
...
# Product URL for "Try it:" links in Twitter/LinkedIn posts (shared with /blog)
